import argparse
from datetime import datetime
import logging
from nt_manager import NTManager
from multiprocessing import Process
from particle_filter import ParticleFilter
from plotter3d import Plotter3d
from logger import Logger
import time

NUM_PARTICLES = 1000


class Quixsam:
    def __init__(self, server=None, save_logs=True):
        self.server = server
        self.save_logs = save_logs

    def run(self):
        # Start plotter thread
        plotter = Plotter3d(NUM_PARTICLES)
        plotter_queue, plotter_targets_queue, plotter_cameras_queue = plotter.start()

        # Continuously reset and try to reconnect if disconnected
        while True:
            nt_manager = NTManager(self.server)

            pf = None
            start_time = None
            logger = None
            while pf is None:
                logging.info("Waiting for targets...")
                retroreflective_targets, apriltag_targets = nt_manager.get_targets()
                if not retroreflective_targets and not apriltag_targets:
                    time.sleep(0.1)
                    continue

                logging.info("Waiting for camera infos...")
                camera_infos = nt_manager.get_camera_infos()
                if not camera_infos:
                    time.sleep(0.1)
                    continue

                plotter_targets_queue.put((retroreflective_targets, apriltag_targets))
                plotter_cameras_queue.put(camera_infos)

                odometry = None
                while odometry is None:
                    logging.info("Waiting for first odometry...")
                    _, odometry, _ = nt_manager.get_next()
                    if odometry is None:
                        time.sleep(0.1)
                        continue

                    pf = ParticleFilter(
                        NUM_PARTICLES,
                        odometry,
                        retroreflective_targets,
                        apriltag_targets,
                    )
                    logging.info("First odometry received!")
                    start_time = time.perf_counter()

                    # Start new logger every time we connect. uncomment out if needed. + uncomment out logger code below
                    # if self.save_logs:
                    #     try:
                    #         now_str = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
                    #         logger = Logger(f"quixsamlog_{now_str}")
                    #         logger.init(
                    #             NUM_PARTICLES,
                    #             odometry,
                    #             retroreflective_targets,
                    #             apriltag_targets,
                    #         )
                    #     except Exception as e:
                    #         print("Logger failed to start")
                    #         print(e)

            while nt_manager.is_connected():
                plotter_cameras_queue.put(nt_manager.get_camera_infos())
                measurement_id, odometry, vision = nt_manager.get_next()
                if odometry is not None:
                    s = time.perf_counter()
                    pf.predict(odometry)
                    has_vision = pf.update(vision, camera_infos)

                    # Publish estimate
                    best_estimate = pf.get_best_estimate()
                    x, y, theta = best_estimate
                    nt_manager.publish_estimate(measurement_id, x, y, theta, has_vision)

                    # Log for resimulation uncomment out for logging
                    # if logger is not None:
                    #     try:
                    #         logger.log(
                    #             s - start_time,
                    #             odometry,
                    #             vision,
                    #             best_estimate,
                    #             has_vision,
                    #         )
                    #     except Exception as e:
                    #         print("Log failed.")

                    # Handle plotting
                    plotter_queue.put((vision, pf.particles, best_estimate, has_vision))
                    logging.info(
                        "Iter {} time: {:.3f} {}".format(
                            measurement_id,
                            time.perf_counter() - s,
                            "VISION" if has_vision else "",
                        )
                    )
                time.sleep(0.001)


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--local", action="store_true")
    args = parser.parse_args()
    Quixsam(
        "10.24.71.2",
        save_logs=not args.local,
    ).run()
