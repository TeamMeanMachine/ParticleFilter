...
def cart2bepinhole(x: float, y: float, z: float):
    """Converts the direction of a 3D cartesian point to a bearing and elevation in a pinhole model."""
    bearing = np.arctan2(y, x)
    elevation = np.arctan2(z, np.sqrt(x * x + y * y))
    return np.hstack((bearing, elevation))
...
def get_tag_corner_offset_T(target_corner_id, target_size):
    OFFSETS = [
